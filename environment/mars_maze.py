...
def action2dxy(a,x,y):
    ...
    seed(int(se))
    shuffle(action)
    return {'0': [0, 1], '1': [1, 0], '2': [0, -1], '3': [-1, 0], '4': [0, 0]}.get(str(action[a]),[0,0])

def observation2dxy(a):
...
def build_asset():
    ...
    print('Asset Build')
    torch.save({'state_graph': state_graph, 'V_size': len(vis), 'E_size': edge_count},
               'environment/asset/' + get_config()['task_name'])
...
def build_IL_dataset():
    ...
    for map in range(num_map):
        ...
        for traj in range(num_traj):
            ...
            # print(X[map,traj,:,:,0])
        # print(S,A,O)
    torch.save({'X': X, 'B0': B0, 'S': S, 'A': A, 'O': O, 'goal': goal_data, 'path_data': path_data},
               'IL_dataset/' + config['task_name'])
